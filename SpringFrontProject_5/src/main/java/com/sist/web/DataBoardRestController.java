...
public class DataBoardRestController {
  ...
  public String databoard_list(int page) throws Exception
  {
	  ...
	  // 자바 스크립트 연결 => Map을 JSON => Kotlin , Flutter
  ...
  /*
   *   C:\\springDev\\springStudy\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp0\\wtpwebapps\\SpringFrontProject_5\\upload
   */
  ...
  public String databoard_delete(int no,String pwd,HttpServletRequest request)
  {
	  ...
	  if(result.equals("yes"))
	  {
		  String path=request.getSession().getServletContext().getRealPath("/")+"upload\\";
		  path=path.replace("\\", File.separator);
		  if(vo.getFilecount()>0) // 파일이 존재할 때
		  {
