 ...
 *                        
 *        == 클래스를 관리하기 위해서는 객체 생성 / 객체 소멸 (객체의 생명주기 관리)
 *        == 객체 생성시에 멤버 변수의 초기화가 필요 시에 사용
 *        == 변수의 초기화
 *          ========== setterDI => p:name => setName()
 *          ========== 생성자 DI  => c:
 *      = AOP : 공동모듈 : 실행시마다 호출하는 기능이 있는 경우에 자동 호출이 가능
 ...
 *                   => 사용자의 요청 받아서 처리 결과를 JSP로 전송하는 역할
 *                                              === @Controller
 ...
 *                                              => @PostMapping(URI)
 *                                              => @RequestMapping(URI)
 *        사용자 요청 ==> DispatcherServlet => HandlerMapping => Model처리   
 *                                                              | request         
 *                                                         ViewResolver
 *                                                           = p:prefix => 경로명
 *                                                           = p:suffix => 확장자
